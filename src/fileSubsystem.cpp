...
bool Server::FileSubsystem::init()
{
...
bool Server::FileSubsystem::bindUserPersonalFile (const std::string& login, std::fstream& file)
{
...
bool Server::FileSubsystem::open_file(std::fstream& file, const string& filename)
{
	...
	if (parrent_path.size()) {
		...
		if (err.value()) {
			...
			std::cerr << diagnosticMessageHead() << err.message() << std::endl;
			std::cerr << "Cannot create parrent directory '" << parrent_path << "' for relations files" << std::endl;
			return false;
	...
	if (file)
		set_permissions(filename);
			...
			if (file) {
				...
				//std::cout << diagnosticMessageHead() << filename << ": File not found. New file exist." << std::endl;
				set_permissions(filename);
				return true;

			} else {

				std::cerr << diagnosticMessageHead() << filename << ": " << std::strerror(errno) << std::endl;
...
void Server::FileSubsystem::set_permissions(const std::string& filename)
{
...
const char* Server::FileSubsystem::diagnosticMessageHead()
{
	const char* message = "File subsystem: ";
	return message;
}
